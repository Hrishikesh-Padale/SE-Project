...
class interface:
    ...
    def generate_board_coordinates(self):
        self.xstart = self.width-(self.width*98)//100
        self.xend = self.width-(self.width*30)//100
        self.boardwidth = self.xend - self.xstart - 0.5
...
print(Interface.boxwidth,Interface.boxheight)
for i in range(8):
	for j in range(8):
...
while running:
    ...
    screen.blit(W_king_image,(423,662))
    clock.tick(60)
    pygame.display.flip()
